...
#region Our Calls
        ...
        public List<Order> MarketClosePosition(string Symbol, string Side, int Size)
        {
            var param = new Dictionary<string, string>();
            param["symbol"] = Symbol;
            param["side"] = Side;
            param["orderQty"] = Size.ToString();
            param["ordType"] = "Market";
            ...
            {
                errors.Add(res);
                Thread.Sleep(HFTMEX.Properties.Settings.Default.RetryAttemptWaitTime); // Force app to wait 500ms
...
*/
        ...
        private int RetryAttempts(string res)
        {
            ...
            {
                if(HFTMEX.Properties.Settings.Default.OverloadRetry)
                {
                    att = HFTMEX.Properties.Settings.Default.OverloadRetryAttempts;
                }
                else
                {
                    att = 0;
